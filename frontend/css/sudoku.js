...
const render = {
    initDigits: (texts) => {
        allTexts = texts.concat(fill(Array(ALL_DIGITS.length - texts.length), ''))
        shuffle(dom.digits).forEach((digit, i) => {
    ...
    updateDigitStatus: (text, isAnswer) => {
        if (isAnswer) {
            let digit = find(dom.digits, x => (x.innerText == ''))
            digit.innerText = text
            digit.className = 'correct'
        }
        else {
            find(dom.digits, x => (x.innerText == text)).className = 'wrong'
        }
    },
    updateTime: (value) => {
...
async function newRound() {
    ...
    digits = xdshuffle(ALL_DIGITS).map((x, i) => {
        return {
            text: x,
    ...
    render.initDigits(filter(digits, x => !x.isAnwser).map(x => x.text))
...
function pressKey(e) {
    ...
    let digit = find(digits, x => (x.text == e.key))
    ...
    let hasPressedAllAnswerDigits = (filter(digits, (x) => (x.isAnwser && x.isPressed)).length == answerCount)
